...
attend %>% 
  ...
  ggplot(aes(x=event, y=number_people, fill = status)) + 
    geom_col()+ theme(axis.text.x = element_text(angle = 45, size = 10, hjust = 1)) +labs(title = "Total Attendance Per Event", x= "Event Name", y="Attendance")
...
attend %>% 
  ...
  summarize(number_status = n()) %>% 
   ggplot(aes(x=reorder(status, -number_status), y=number_status)) + 
    geom_col()+ theme(axis.text.x = element_text(angle = 45, size = 10, hjust = 1)) + labs(title = "Total Attendance Per Date", x = "Status", y="Attendance") +geom_text (aes(label=number_status), vjust= -0.5)
```
